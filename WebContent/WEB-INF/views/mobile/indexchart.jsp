...
<script type="text/javascript" src="${ctx}/static/public/salt-router.js"></script>
...
function binddatalist(){
	...
	$.ajax({
	  	  ...
	  	  success: function(data){
	  		  ...
	  		  var datalist="";
				  $.each(data.userdata,function(i,n){
			  			datalist+="<a class=\"weui_cell\" onclick=gocitypage(\"${ctx}/data/citypage?pname="+encodeURI(n.provincename)+"&color=\",\""+n.id+"\") >";
...
function getoption(xAxisData,yAxisData,weekdata){
	var dataoption = {
			tooltip: {
				formatter: function(obj) {
					var value = obj.value;
					return '<div style="border-bottom: 1px solid #777; font-size: 12px;padding-bottom: 2px;">'  + value[1] + value[0] + '日：' + value[2] + '%</div>采集率：' + value[2] + '%<br>安装量：'+ value[3] + '<br>排名：第'+ value[4]
				}
			},
			xAxis: {
				type: 'category',
